...
const landscapeObject = {
  ...
  3: {
    ...
    title: "DUTCH MANSION.",
    description: "consectetur adipisc",
  },
  4: {
...
const Landscape3D = () => {
  ...
  useEffect(() => {
    ...
    renderer.setSize(window.innerWidth * 0.90, window.innerHeight * 0.85);
  ...
  return <div className='flex items-center justify-center w-screen h-screen'>
    ...
    <div ref={containerRef} className='relative overflow-hidden rounded-3xl' >
      <div className='absolute flex flex-col gap-2 top-4 right-4'>
        <div className='px-4 py-2 font-medium text-white duration-300 ease-linear bg-black rounded-full top-4 right-8 bg-opacity-60 hover:cursor-pointer hover:bg-white hover:text-black hover:bg-opacity-100'
          onClick={() => window.location.reload(false)}
        >Refresh for Random</div>
      </div>
      ...
      <div className='absolute w-4/6 text-white bottom-6 left-6'>
        ...
        <h4 className='text-base text-white font-PaulGrotesk'>{landscapeObject[randNumber].description}</h4>
      </div>
      <div className='absolute px-4 py-2 text-white duration-500 ease-in-out opacity-50 bottom-4 right-10 hover:bg-black hover:bg-opacity-70 hover:rounded hover:opacity-80'>Credits to <a className='underline cursor-pointer underline-offset-2' href='https://skybox.blockadelabs.com/'>BlockadeLabs.com</a></div>
    </div>
  </div >
};
